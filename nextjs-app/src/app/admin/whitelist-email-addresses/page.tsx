    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-1">
        <h1 className="text-3xl font-black tracking-tight text-white uppercase italic">
          Email <span className="bg-gradient-to-r from-purple-400 to-fuchsia-400 bg-clip-text text-transparent">Whitelist</span>
        </h1>
        <p className="text-sm font-medium text-zinc-500 uppercase tracking-[0.2em]">
          {(emails?.length ?? 0).toLocaleString()} Whitelisted Addresses
        </p>
      </div>

      {/* Add form */}
      <div className="bg-zinc-900/50 border border-white/5 rounded-3xl p-6 backdrop-blur-sm shadow-2xl">
        <p className="text-xs font-black uppercase tracking-widest text-zinc-400 mb-4">Add Email Address</p>
        <form action={createEmail} className="flex items-center gap-3">
          <input
            type="email"
            name="email_address"
            required
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.02]">
                ...
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-zinc-500">Modified</th>
