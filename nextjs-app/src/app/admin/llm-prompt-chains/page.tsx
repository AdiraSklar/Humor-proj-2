    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-1">
        <h1 className="text-3xl font-black tracking-tight text-white uppercase italic">
          Prompt <span className="bg-gradient-to-r from-purple-400 to-fuchsia-400 bg-clip-text text-transparent">Chains</span>
        </h1>
        <p className="text-sm font-medium text-zinc-500 uppercase tracking-[0.2em]">
          {(count ?? 0).toLocaleString()} Chains · Read-only
        </p>
      </div>
      ...
      <div className="bg-zinc-900/50 border border-white/5 rounded-3xl p-6 backdrop-blur-sm shadow-2xl">
        {error && (
          <p className="mb-4 rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-400">
            Failed to load chains: {error.message}
          </p>
        )}
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.02]">
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-zinc-500">Chain ID</th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-zinc-500">Caption Request</th>
            ...
            <tbody className="divide-y divide-white/5">
              {!chains?.length ? (
                <tr>
                  <td colSpan={3} className="px-6 py-16 text-center text-zinc-500 italic">
                ...
                chains.map((c) => (
                  <tr key={c.id} className="group transition-colors hover:bg-white/[0.02]">
                    <td className="px-6 py-4 font-mono text-xs text-zinc-400 font-bold">{c.id}</td>
                    <td className="px-6 py-4">
                      <Link
                        href={`/admin/caption-requests`}
                        className="font-mono text-xs text-zinc-500 group-hover:text-violet-400 transition-colors"
                      >
                        #{c.caption_request_id}
                      </Link>
                    </td>
                    <td className="px-6 py-4 text-right text-xs text-zinc-500 tabular-nums">{formatDate(c.created_datetime_utc)}</td>
                  </tr>
                ))
              )}
            </tbody>
